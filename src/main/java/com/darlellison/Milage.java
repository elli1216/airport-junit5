...
public class Milage {
    ...
    public void calculateGivenPoints() {
        for (Passenger passenger: passengersMilageMap.keySet()) {
            if (passenger.isVip()) {
                passengersMilageMap.put(passenger, passengersMilageMap.get(passenger) / VIP_FACTOR);
            } else {
                passengersMilageMap.put(passenger, passengersMilageMap.get(passenger) / USUAL_FACTOR);
            }
        }
    }
}
